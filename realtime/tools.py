...
from .utils import ModelName, model_name_to_id
import logging
...
{memory_content}
    ...
    elements = [cl.File(name=response.file_name, path=scratch_pad_dir + "/" + response.file_name, display="inline")]
...
async def update_file_handler(prompt: str, model: ModelName = ModelName.base_model) -> dict:
    """
...
tools = [query_stock_price, draw_plotly_chart, create_file, update_file, delete_file]